							 ...
							 RPCparas *pRpcs, RFMCalcPara &para, double* pResErr, RFMType rfmType )
{
	...
	//gsl_matrix* M_mat = gsl_matrix_alloc(nImgPts, 39);
	//gsl_matrix* M_mat_cpy = gsl_matrix_alloc(nImgPts, 39);
	//gsl_matrix* N_mat = gsl_matrix_alloc(nImgPts, 39);
	//gsl_vector* row_M = gsl_vector_alloc(nImgPts);
	//gsl_vector* col_N = gsl_vector_alloc(nImgPts);

	M_mat.setZero();
	N_mat.setZero();
	row_M.setZero();
	col_N.setZero();
	//gsl_matrix_set_all(M_mat, 0.0);
	//gsl_matrix_set_all(N_mat, 0.0);
	//gsl_vector_set_all(row_M, 0.0);
	//gsl_vector_set_all(col_N, 0.0);
	...
	for(int i=0; i<nImgPts; i++)
	{
		...
		//	gsl_matrix_set(M_mat, i, j,  (-1.0*iX) * gsl_matrix_get(M_mat, i, (j -19)) );
		}
	}
/*
	for(int i=0; i<nImgPts; i++)
	{
		...
		iX = (pImgPts[2*i] - pRpcs->dXOff) / pRpcs->dXScale;
		iY = (pImgPts[2*i] - pRpcs->dYOff) / pRpcs->dYScale;
		gsl_vector_set(col_N, i, iY);

		setFormerMat(N_mat, i, cX, cY, cZ);

		for(int j=20; j<39; j++)
		{
			gsl_matrix_set(N_mat, i, j,  (-1.0*iY) * gsl_matrix_get(N_mat, i, (j -19)) );
		}
	}
*/
	VectorXd r_coeff(39);
...
/*
	gsl_matrix* workMat = gsl_matrix_alloc(M_mat->size2, M_mat->size1);
	gsl_matrix* weight_M  = gsl_matrix_alloc(M_mat->size1 , M_mat->size1);
	gsl_matrix* weight_N  = gsl_matrix_alloc(M_mat->size1 , M_mat->size1);
	gsl_matrix* not_squared_weight_M  = gsl_matrix_alloc(M_mat->size1 , M_mat->size1);
	gsl_matrix* M_weighted = gsl_matrix_alloc(M_mat->size2, M_mat->size2);
	gsl_matrix* N_weighted = gsl_matrix_alloc(N_mat->size2, N_mat->size2);
	gsl_vector* row_M_weighted = gsl_vector_alloc(M_mat->size2);
	gsl_vector* col_N_weighted = gsl_vector_alloc(N_mat->size2);
	gsl_vector* error_vector = gsl_vector_alloc(M_mat->size1);
	gsl_matrix_set_all(weight_M, 0.0);
	gsl_matrix_set_all(weight_N, 0.0);
*/
	...
	//gsl_matrix_set_all(workMat, 0.0);
	//gsl_vector_set_all(error_vector, 0.0);

	workMat = M_mat.transpose() * weight_M;
	row_M_weighted = workMat * row_M;
	M_weighted = workMat * M_mat;
	//gsl_blas_dgemm (CblasTrans, CblasNoTrans, 1.0, M_mat, weight_M, 0.0, workMat);
	//gsl_blas_dgemv( CblasNoTrans, 1.0, workMat, row_M, 0.0, row_M_weighted );
	//gsl_blas_dgemm (CblasNoTrans, CblasNoTrans, 1.0, workMat, M_mat, 0.0, M_weighted);
	...
	//gsl_matrix_set_all(workMat, 0.0);
	//gsl_blas_dgemm (CblasTrans, CblasNoTrans, 1.0, N_mat, weight_N, 0.0, workMat);
	//gsl_blas_dgemv( CblasNoTrans, 1.0, workMat, col_N, 0.0, col_N_weighted );
	//gsl_blas_dgemm (CblasNoTrans, CblasNoTrans, 1.0, workMat, N_mat, 0.0, N_weighted);

	r_coeff = solveMat(M_weighted, row_M_weighted);
	c_coeff = solveMat(N_weighted, col_N_weighted);
	//solveMat(M_weighted, row_M_weighted, r_coeff);
	//solveMat(N_weighted, col_N_weighted, c_coeff);

	error_vector = weight_M * row_M;
	M_mat_cpy = weight_M * M_mat;
	error_vector = (M_mat_cpy * r_coeff) - error_vector;
	//gsl_blas_dgemv(CblasNoTrans, 1.0, weight_M, row_M, 0.0, error_vector);
	//gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, weight_M, M_mat, 0.0, M_mat_cpy);
	//gsl_blas_dgemv(CblasNoTrans, 1.0, M_mat_cpy, r_coeff, -1.0, error_vector);
	...
	//weight_M = gsl_matrix_alloc(nImgPts , nImgPts);
	//weight_N  = gsl_matrix_alloc(nImgPts , nImgPts);
	int count = 0;
	do
	{
		weight_M.setZero();
...
/*
void squareMat(gsl_matrix* mat)
{
	for(int i=0; i<mat->size1; i++){
		double v = gsl_matrix_get(mat, i,i);
		v = v*v;
		gsl_matrix_set(mat, i, i, v);
	}
}
*/
